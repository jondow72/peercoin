...
int64_t GetWeight(int64_t nIntervalBeginning, int64_t nIntervalEnd)
{
    ...
    return min(nIntervalEnd - nIntervalBeginning - GetStakeMinAge(nIntervalEnd), (int64_t)nStakeMaxAge);
}

// Get the last stake modifier and its generation time from a given block
static bool GetLastStakeModifier(const CBlockIndex* pindex, uint64& nStakeModifier, int64_t& nModifierTime)
{
    if (!pindex)
...
static bool SelectBlockFromCandidates(
    ...
    int64_t nSelectionIntervalStop, uint64 nStakeModifierPrev,
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    if (fDebug)
    {
        LogPrintf("ComputeNextStakeModifier: prev modifier=0x%016"PRI64x" time=%s\n", nStakeModifier, DateTimeStrFormat(nModifierTime).c_str());
    }
...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    nStakeModifier = 0;
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
        ...
        {   // reached best block; may happen if node is behind on block chain
			...
			{
			if(fDebug)
				 LogPrintf(">> nStakeModifierTime = %"PRId64", pindexFrom->GetBlockTime() = %"PRId64", nStakeModifierSelectionInterval = %"PRId64"\n",
...
bool CheckStakeKernelHash(CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    if(!pindexPrev && fDebugMagi) LogPrintf("ERROR: CheckStakeKernelHash() - pindexPrev is null");
...
//    int64_t nTimeWeight = min((int64_t)nTimeTx - txPrev.nTime, (int64_t)nStakeMaxAge) - nStakeMinAge;
	...
	// LogPrintf(">>> CheckStakeKernelHash: nTimeWeight = %"PRId64"\n", nTimeWeight);
    // Calculate hash
    CDataStream ss(SER_GETHASH, 0);
    ...
    if (CBigNum(hashProofOfStake) > bnCoinDayWeight * bnTargetPerCoinDay)
	{
		if(fDebug)
		{
			LogPrintf(">>> nValueIn = %"PRId64", nTimeWeight = %"PRId64", nPrevMoneySupply = %"PRId64"\n", nValueIn, nTimeWeight, pindexPrev->nMoneySupply);
...
//			LogPrintf(">>> hashTarget = %s, hashProof = %s\n",
...
//			bnCoinDayWeight.getuint256().ToString().c_str(), bnTargetPerCoinDay.getuint256().ToString().c_str());
    ...
    if (fDebug && !fPrintProofOfStake)
    {
		...
		if(fDebug)
		{
			LogPrintf(">>> nValueIn = %"PRId64", nTimeWeight = %"PRId64"\n", nValueIn, nTimeWeight);
			CBigNum hashTargett_ = bnCoinDayWeight * bnTargetPerCoinDay;
			LogPrintf(">>> bnCoinDayWeight = %s, bnTargetPerCoinDay = %s\n",
			bnCoinDayWeight.ToString().c_str(), bnTargetPerCoinDay.ToString().c_str());
//			LogPrintf(">>> hashTarget = %s, hashProof = %s\n",
