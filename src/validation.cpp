...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (tx.nTime > nTime + (IsProtocolV09(nTime) ? MAX_FUTURE_BLOCK_TIME : MAX_FUTURE_BLOCK_TIME_PREV9))
    ...
    std::string reason;
    if (m_pool.m_require_standard && !IsStandardTx(tx, m_pool.m_max_datacarrier_bytes, m_pool.m_permit_bare_multisig, reason)) {
    ...
    if (!Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees, Params().GetConsensus(), tx.nTime ? tx.nTime : nTime)) {
        return false; // state filled in by CheckTxInputs
    }

    if (ws.m_base_fees < GetMinFee(tx, tx.nTime ? tx.nTime : nTime))
...
int64_t GetProofOfWorkReward(unsigned int nBits, int nHeight, int64_t nFees)
{
    bool fTestNet = gArgs.GetBoolArg("-testnet", false);
...
bool Chainstate::IsInitialBlockDownload() const
{
    ...
    if (m_chain.Tip() == nullptr)
        return true;
    if (m_chain.Tip()->nChainTrust < m_chainman.MinimumChainWork()) {
        return true;
    }
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    if (block_index.pprev && IsProtocolV12(block_index.pprev)) {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) {  // PoS block
        ...
        if (IsPoSIIProtocolV2(nHeight)) {
            // Compute nCoinAge V2 using view (implement GetCoinAgeV2 as free function if not member)
            fTxGetCoinAge = GetCoinAgeV2(block.vtx[1].get(), view, nCoinAge, nHeight);  // Pass raw ptr
        } else {
            // Compute nCoinAge V1 using view
            fTxGetCoinAge = GetCoinAge(block.vtx[1].get(), view, nCoinAge, nHeight);  // Pass raw ptr
        }
        if (!fTxGetCoinAge) {
        ...
        // Actual mint: Outputs - inputs (use GetValueOut; compute GetValueIn from view)
        CAmount nValueIn = 0;
        for (const auto& txin : block.vtx[1]->vin) {
            const Coin& coin = view.AccessCoin(txin.prevout);
            if (coin.IsSpent()) return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-tx-inputs", "spent input in coinstake");
            nValueIn += coin.out.nValue;
        }
        nStakeReward = block.vtx[1]->GetValueOut() - nValueIn;

        // Expected reward (use pindex->pprev for prev chain state; nFees from txs)
        int64_t nPoSReward = GetProofOfStakeReward(static_cast<int64_t>(nCoinAge), nFees, pindex->pprev);
        ...
        // Update pindex for stake (assume ComputeNextStakeModifier returns uint256; use .GetUint64() or cast)
        uint256 stakeMod = ComputeNextStakeModifier(pindex->pprev, block.GetBlockTime());
        pindex->nStakeModifier = stakeMod.GetUint64();  // Or static_cast<uint64_t>(stakeMod.GetCompact()) if needed
    }
...
arith_uint256 CalculateHeadersWork(const std::vector<CBlockHeader>& headers)
{
    ...
    for (const CBlockHeader& header : headers) {
        ...
        total_work += GetBlockTrust(dummy);
    }
    return total_work;
}
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    if (!CheckWitnessMalleation(block, (pindexPrev && IsBTC16BIPsEnabled(pindexPrev->nTime)), state)) {
        return false;
    }
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    const auto& chainstate = ActiveChainstate();
    {
        ...
        if (m_best_header->nChainTrust >= UintToArith256(GetConsensus().nMinimumChainWork)) return;
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block, CBlockIndex** ppindex, bool* fPoSDuplicate)
{
    AssertLockNotHeld(cs_main);
...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        ...
        for (CBlockIndex* pindex : vSortedByHeight) {
            ...
            if (pindex->nStatus & BLOCK_FAILED_MASK && (!m_best_invalid || pindex->nChainTrust > m_best_invalid->nChainTrust)) {
