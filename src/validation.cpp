...
int64_t GetProofOfWorkReward(unsigned int nBits, CBlockIndex* pindexPrev = nullptr)
{
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    if (block.vtx[0]->GetValueOut() > (block.IsProofOfWork() ? (GetProofOfWorkReward(block.nBits, pindexPrev) - nCoinbaseCost) : 0))
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount",
                strprintf("CheckBlock() : coinbase reward exceeded %s > %s",
                   FormatMoney(block.vtx[0]->GetValueOut()),
                   FormatMoney(block.IsProofOfWork() ? GetProofOfWorkReward(block.nBits, pindexPrev) : 0)));
    // Check transactions
